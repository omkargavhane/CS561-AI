'''
Title : Multivariate Naive bayes for given documents
...
def vectorize(train_d,train_vocab):
    ...
    cnt=0
    for di in train_d:
        vectors.append([1 if train_vocab[i] in di else 0 for i in range(len(train_vocab))])
        vectors[-1].append(Class[cnt])
        cnt+=1
    return vectors
...
def nb_multivariate():
    ...
    for d in test_d:
        ...
        for word in train_vocab:
            ...
            prob_0_given_test*=(words_prob[word][0]**b)*((1-words_prob[word][0])**(1-b))
            prob_1_given_test*=(words_prob[word][1]**b)*((1-words_prob[word][1])**(1-b))
    ...
    print("Vocabulary:")
    print(train_vocab)
    #printing of test document and target classes
